...
(function() {
    ...
    const SYNTAX_RULES = {
        html: { tags: /(<\/?\w+[\s\S]*?>)/g, attributes: /(\b\w+)=/g, strings: /("|')([^"']*)("|')/g, comments: /()/g, doctype: /(<!DOCTYPE[\s\S]*?>)/g },
        css: { selectors: /(.+?)(?={)/g, properties: /(\b\w+-?[\w-]*):/g, values: /:([\s\S]+?)(?=;)/g, comments: /(\/\*[\s\S]*?\*\/)/g, strings: /("|')([^"']*)("|')/g },
        js: { keywords: /\b(let|const|var|function|class|return|if|else|for|while|do|import|export|from|async|await|new|this|try|catch|finally|throw|switch|case|break)\b/g, functions: /(\b\w+)\s*(?=\()/g, classes: /\b(String|Number|Boolean|Array|Object|Promise|Symbol|Map|Set|RegExp|Error)\b/g, constants: /(\b[A-Z_]+)\b/g, strings: /("|`)(?:\\.|[^"\\])*(")/g, comments: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
        json: { keys: /"([^"]+)":/g, strings: /"([^"]+)"/g, numbers: /(\b\d+(\.\d+)?\b)/g, booleans: /\b(true|false|null)\b/g },
        md: { headings: /(#+.*)/g, bold: /(\*\*|__)(.*?)\1/g, italic: /(\*|_)(.*?)\1/g, links: /(\[.*?\]\(.*?\))/g },
        ts: { keywords: /\b(let|const|var|function|class|return|if|else|for|while|do|import|export|from|async|await|new|this|try|catch|finally|throw|switch|case|break|interface|type|public|private|protected)\b/g, types: /\b(string|number|boolean|any|void|never|unknown)\b/g, functions: /(\b\w+)\s*(?=\()/g, strings: /("|`)(?:\\.|[^"\\])*(")/g, comments: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
        jsx: { keywords: /\b(let|const|var|function|class|return|if|else|for|while|do|import|export|from|async|await|new|this|try|catch|finally|throw|switch|case|break)\b/g, tags: /(<\/?\w+[\s\S]*?>)/g, attributes: /(\b\w+)=/g, strings: /("|')([^"']*)("|')/g, comments: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
        vue: { tags: /(<\/?\w+[\s\S]*?>)/g, attributes: /(\b\w+)=/g, strings: /("|')([^"']*)("|')/g, comments: /()/g },
        scss: { selectors: /(.+?)(?={)/g, properties: /(\b\w+-?[\w-]*):/g, values: /:([\s\S]+?)(?=;)/g, comments: /(\/\*[\s\S]*?\*\/)/g, variables: /(\$\w+)/g, strings: /("|')([^"']*)("|')/g },
        svg: { tags: /(<\/?\w+[\s\S]*?>)/g, attributes: /(\b\w+)=/g, strings: /("|')([^"']*)("|')/g, comments: /()/g },
    };

    const getFileType = (fileName) => {
        const ext = fileName.split('.').pop().toLowerCase();
        return { yml: 'yaml', tsx: 'jsx', sass: 'scss' }[ext] || ext;
    };

    const getFileIconClass = (fileType) => {
        const classes = { html: 'html', css: 'css', js: 'js', json: 'json', yaml: 'yaml', md: 'md', ts: 'ts', jsx: 'jsx', vue: 'vue', scss: 'scss', svg: 'svg', folder: 'folder' };
